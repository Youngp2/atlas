...
class Order(models.Model):
    ...
    tracking_number = models.CharField(default=f"ATLAS{str(uuid.uuid4())[:18]}-ORDER", max_length=30, unique=True)
    ...
    def __str__(self):
        return f"{self.reciever_name} -Order- {self.get_status_display()}"
        
        
    def generate_tracking_number(self):
        uuid_string = str(uuid.uuid4().int)
        formatted_uuid = f"ATLAS{str(uuid.uuid4())[:18]}-ORDER"
